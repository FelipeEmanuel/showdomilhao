...
class QuizComumSeries extends React.Component<quizSeriesProps, quizSeriesState> {
  countdownInterval: number | undefined;

  constructor(props: quizSeriesProps) {
    super(props);
  ...
  componentDidMount() {
    axios
    .get("http://localhost:3000/usedQuestionsSeries")
    .then((response) => {
    ...
    axios
      .get("http://localhost:3000/questionsSeries")
      .then((response) => {
  ...
  handleNextQuestion = () => {
    ...
    if (currentQuestion) {
      axios.post("http://localhost:3000/usedQuestionsSeries", { questionId: currentQuestion.id })
        .then((response) => {
  ...
  resetUsedQuestions = () => {
    axios.get("http://localhost:3000/usedQuestionsSeries")
      .then((response) => {
        const usedQuestions = response.data; // Obtém todos os itens usados
        const deleteRequests = usedQuestions.map(question => 
          axios.delete(`http://localhost:3000/usedQuestionsSeries/${question.id}`) // Deleta cada item
        );
...
export default QuizComumSeries;
