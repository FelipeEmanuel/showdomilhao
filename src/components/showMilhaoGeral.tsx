...
export default function ShowMilhaoGeral({ onBack }: showMilhaoGeralProps) {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:3000/questionsGerais")
      .then((response) => {
  ...
  const postRanking = (index: number) => {
    axios
      .post("http://localhost:3000/rankingMilhaoGeral", {
        nome: playerName,
        index: index,
      })
      .then((response) => {
